...
#        The first input required is the directory which has NetCDF files of ETo values for a GCM.
...
def browse_directory():
    ...
    if path:
        ...
        directory_label.config(text=f"Select Directory with ETo files : {directory_path_var_1.get()}")
...
def browse_directory_1():
    ...
    if path2:
        ...
        directory_label_1.config(text=f"Select Directory to store excel files with sum of eto : {directory_path_var_2.get()}")
def finish():
    root.destroy()
...
directory_button_1 = tk.Button(root, text="Browse Directory to save annual sum of ETo", command=browse_directory_1)
...
# This part of the code calculates the annual sum of reference evapotranspiration
...
for file_path in file_paths:
    ...
    sum_etoper_year = np.zeros(len(unique_years))

    for i, year in enumerate(unique_years):
        mask = (years == year)
        sum_etoper_year[i] = np.nansum(eto_var[mask])
...
excel_file_path = excel_file_path+'_' + str(int(start_year) + 9) + '_annual_sum.xlsx'
