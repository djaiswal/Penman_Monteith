...
def browse_file(file_index):
    file_path = filedialog.askopenfilename(
        filetypes=[(start_year+"files", "*"+start_year+"*"), ("Elevation files", "*elevation*")]) # include file containing "elevation" in its name
    if file_path:
...
rsds_data = datarsds.variables["rsds"][:]*0.0864
...
rn_data = calculate_rn(rsds_data, lat_data, temperature_max, temperature_min, ea_data, elevation_data, days_decade)
